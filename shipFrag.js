(function(root) {
  ...
  var shipFrag = Asteroids.shipFrag = function(shipPos, shipAngle, radius, coordinates, pos) {
    this.shipPos = shipPos;
    this.pos = pos;//CENTROID THAT NEEDS TO BE RECALCULATED
    this.vel = 50 * (Math.random() * 0.5 + 0.5);
  ...
  shipFrag.prototype.shatter = function() {
    ...
    newCoords.forEach(function(transCoord) {
      var coordSum = _.reduce(transCoord, function(mem, coord) {
        return [mem[0] + coord[0], mem[1] + coord[1]];
      }, [0,0]);
      ...
      fragments.push(new shipFrag(f.shipPos, f.rotation, f.radius /2, coords, newPos));
    });
    return fragments;
  }
})(this);
